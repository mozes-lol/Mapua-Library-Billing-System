...
function initDashboardPage() {
  ...
  serviceForm.addEventListener("submit", (e) => {
    ...
    if (selectedServices.length === 0) {
      status.textContent = "Please enter quantity for at least one service";
      status.style.color = "red";
      return;
    }
...
function updateTotalAmount() {
  ...
  `;
    ...
    setTimeout(() => {
      ...
      if (finalizeBtn)
        finalizeBtn.addEventListener("click", handleFinalizeInvoice);
    }, 100);
  }
  ...
  async function loadTransactionsTable() {
    ...
    try {
      ...
      if (transactionsError) {
        ...
        const errorMsg =
          '<tr><td colspan="10" style="text-align: center; color: red;">Error loading transactions</td></tr>';
      ...
      const renderTableRows = (transactionList, includeProcessedBy = false) => {
        if (!transactionList || transactionList.length === 0) {
          return `<tr><td colspan="${
            includeProcessedBy ? 11 : 10
          }" style="text-align: center;">No transactions</td></tr>`;
        }
        ...
        return transactionList
          .map((transaction) => {
            ...
            const processedByCell = includeProcessedBy
              ? `<td>${transaction.processed_by || "N/A"}</td>`
              : "";
            const buttonLabel = includeProcessedBy
              ? "View Details"
            ...
            return `
              ...
              transaction.status || "N/A"
                  ...
                  <td>
                      ...
                      })" style="background-color: #2196F3; color: white; padding: 5px 10px; border: none; border-radius: 3px; cursor: pointer;">${buttonLabel}</button>
                  </td>
              </tr>
            `;
          })
    ...
    try {
      ...
      if (transaction.user_id) {
        ...
        if (userError) {
          ...
          userName = `${user.given_name} ${user.middle_name || ""} ${
            user.last_name
          }`
            .replace(/\s+/g, " ")
            .trim();
      ...
      document.getElementById("detailTransactionId").textContent =
        transaction.transaction_id;
      document.getElementById("detailTransactionCode").textContent =
        transaction.transaction_code || "N/A";
      document.getElementById("detailUserName").textContent = userName;
      document.getElementById("detailDateTime").textContent = new Date(
        transaction.date_time
      ).toLocaleString();
      document.getElementById("detailSchoolYear").textContent =
        transaction.school_year || "N/A";
      document.getElementById("detailTerm").textContent =
        transaction.term || "N/A";
      document.getElementById("detailStatus").textContent =
        transaction.status || "N/A";
      document.getElementById("detailProcessedBy").textContent =
        transaction.processed_by || "Not processed yet";
      ...
      if (codeInput) {
        codeInput.oninput = () => {
          ...
          if (codeStatus) {
            if (!hasValue) {
              codeStatus.textContent =
                "Enter the transaction code to continue.";
      ...
      // Show modal
      document.getElementById("transactionDetailsModal").style.display =
        "block";
    } catch (error) {
  ...
  async function handleFinalizeInvoice() {
    ...
    if (!enteredCode) {
      statusElement.textContent =
        "Please enter a transaction code to finalize.";
      statusElement.style.color = "red";
      return;
    }
  ...
  async function loadUsersTable() {
    ...
    try {
      ...
      if (error) {
        ...
        tableBody.innerHTML =
          '<tr><td colspan="10" style="text-align: center; color: red;">Error loading users</td></tr>';
        return;
      }
            ...
            <tr>
                ...
                <td>
                    <button onclick="editUser('${
                      user.user_id
                    }')" style="background-color: #2196F3; color: white; padding: 5px 10px; border: none; border-radius: 3px; cursor: pointer; margin-right: 5px;">Edit</button>
                    <button onclick="deleteUser('${
                      user.user_id
                    }')" style="background-color: #f44336; color: white; padding: 5px 10px; border: none; border-radius: 3px; cursor: pointer;">Delete</button>
                </td>
            </tr>
        `
        )
      ...
      tableBody.innerHTML = services
        .map(
          ...
          <tr>
            ...
            <td>
              <button onclick="editServiceType('${
                service.service_id
              }')" style="background-color: #2196F3; color: white; padding: 5px 10px; border: none; border-radius: 3px; cursor: pointer; margin-right: 5px;">Edit</button>
              <button onclick="deleteServiceType('${
                service.service_id
              }')" style="background-color: #f44336; color: white; padding: 5px 10px; border: none; border-radius: 3px; cursor: pointer;">Delete</button>
            </td>
          </tr>
  ...
  window.editServiceType = async function (serviceId) {
    try {
      ...
      const statusElement = document.getElementById("serviceTypeStatus");
      statusElement.textContent =
        "Error loading service type: " + error.message;
      statusElement.style.color = "red";
    }
  };
  ...
  window.deleteServiceType = async function (serviceId) {
    ...
    try {
      ...
      statusElement.textContent =
        "Error deleting service type: " + error.message;
      statusElement.style.color = "red";
    }
  };
  ...
  async function initTransactionStatusPage() {
    ...
    try {
      ...
      transactions.forEach((tx) => {
        ...
        header.textContent = `Transaction ID: ${tx.transaction_id} | User ID: ${
          tx.user_id
        } | Processed By: ${tx.processed_by || "N/A"} | Status: ${
          tx.status || "N/A"
        } | Date: ${
